...
def capitalAppreciationModel(data, numMonths, monthlyAmount):
  result = {}
  
  for i in xrange(0, len(data) - numMonths):
    result[i] = [[0, 0.0]]
    for j in xrange(i, i + numMonths):
      quantity = monthlyAmount / float(data[j][1])
...
capitalAppreciationModel(workingData, yearsInMonths, monthlyInvestment)
